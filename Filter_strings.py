...
class FilterStrings:
    ...
    def get_file_ending_cleanup_string():
        flag = "-   In case Integrated Wireless Access-point is implemented on DMVPN NG router where ZBF is configured it is then necessary to \
            add SVIs created for wireless networks (e.g. SVI VLAN30 used by HCAccess) into INSIDE security zone as a member!"
